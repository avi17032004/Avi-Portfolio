import React, { useState, useEffect } from "react";

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [hasScrolled, setHasScrolled] = useState(false);

  const toggleSidebar = () => {
    setIsOpen(!isOpen);
  };

  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 10) {
        setHasScrolled(true);
      } else {
        setHasScrolled(false);
      }
    };

    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, []);

  return (
    <div
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        hasScrolled
          ? "bg-gray-900 shadow-md"
          : "bg-transparent"
      }`}
    >
      <div className="flex justify-between items-center p-4 md:ml-16 md:mr-16">
        {/* Logo */}
        <h1 className="text-xl font-bold text-yellow-300">Avinash B</h1>

        {/* Hamburger Icon for mobile */}
        <button
          className="block md:hidden focus:outline-none text-yellow-300"
          onClick={toggleSidebar}
        >
          <div className="h-1 w-6 bg-yellow-300 mb-1"></div>
          <div className="h-1 w-6 bg-yellow-300 mb-1"></div>
          <div className="h-1 w-6 bg-yellow-300"></div>
        </button>

        {/* Desktop Navigation */}
        <div className="hidden md:flex gap-14">
          <ul className="flex gap-14">
            {["home", "about", "skills", "projects", "contact"].map((item) => (
              <li
                key={item}
                className="text-lg font-normal border-transparent hover:border-b-2 hover:text-yellow-300 hover:border-yellow-300"
              >
                <a href={`#${item}`}>{item.charAt(0).toUpperCase() + item.slice(1)}</a>
              </li>
            ))}
          </ul>
        </div>

        {/* Mobile Sidebar */}
        <div
          className={`fixed top-0 right-0 h-full w-46 bg-gray-800 text-white transform ${
            isOpen ? "translate-x-0" : "translate-x-full"
          } transition-transform duration-300 ease-in-out z-50 md:hidden`}
        >
          <button
            className="text-yellow-300 mt-4 text-3xl ml-4 focus:outline-none"
            onClick={toggleSidebar}
          >
            &times;
          </button>
          <ul className="flex flex-col items-start mt-3 gap-4 ml-4">
            {["home", "about", "skills", "projects", "contact"].map((item) => (
              <li
                key={item}
                onClick={toggleSidebar}
                className="text-base font-normal hover:text-yellow-300"
              >
                <a href={`#${item}`}>{item.charAt(0).toUpperCase() + item.slice(1)}</a>
              </li>
            ))}
          </ul>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
